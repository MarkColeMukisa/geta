        ...
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            @forelse($tenants as $t)
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    <td class="px-3 py-2">{!! $search ? preg_replace('/('.preg_quote($search,'/').')/i','<mark class="bg-yellow-200 dark:bg-yellow-600/60 px-0.5 rounded">$1</mark>', e($t->name)) : e($t->name) !!}</td>
                    <td class="px-3 py-2">{!! $search ? preg_replace('/('.preg_quote($search,'/').')/i','<mark class="bg-yellow-200 dark:bg-yellow-600/60 px-0.5 rounded">$1</mark>', e($t->contact)) : e($t->contact) !!}</td>
                    <td class="px-3 py-2">{!! $search ? preg_replace('/('.preg_quote($search,'/').')/i','<mark class="bg-yellow-200 dark:bg-yellow-600/60 px-0.5 rounded">$1</mark>', e($t->room_number)) : e($t->room_number) !!}</td>
                    <td class="px-3 py-2 text-xs">{{ $t->created_at->diffForHumans() }}</td>
                    <td class="px-3 py-2 space-x-2">
                            @can('manage-tenants')
                                <button wire:click="edit({{ $t->id }})" class="text-indigo-600 hover:underline">Edit</button>
                                <button wire:click="delete({{ $t->id }})" class="text-red-600 hover:underline" onclick="return confirm('Delete tenant {{ addslashes($t->name) }}?')">Delete</button>
                            @endcan
                    </td>
                    <td class="px-3 py-2">
                        <button type="button" wire:click="openHistory({{ $t->id }})" class="text-orange-600 hover:underline">View</button>
                    </td>
                </tr>
            @empty
                <tr>
